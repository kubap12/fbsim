...
CREATE_DATA=True #Czy generować dane
HFA_TOGGLE=False  #Czy gospodarz ma posiadać przewagę
BOOTSTRAP_TOGGLE=False #Czy elo na początku sezonu ma być losowo zmienione
BOOTSTRAP_VAL=500 #Jaki ma być zakres zmian elo
...
class Klub(object):
  ...
  def reset(self):
    self.elo = self.elo_const + BOOTSTRAP_TOGGLE*BOOTSTRAP_VAL*np.random.uniform()
    self.wins = 0
    self.draws = 0
    self.loses = 0
